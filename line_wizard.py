...
_STEP_DEFINITIONS = [
    ("1 / 7", "Kriterler", "Debi ve basinc bilgilerini girin. Mobil uygulamadaki LINE fan hesap mantigi kullanilir."),
    ("2 / 7", "Fan Secimi", "Uygun fan tipi ve motor gucunu secin."),
    ("3 / 7", "Kartus Filtre", "Filtre medyasi ve LINE kasasini secin."),
    ("4 / 7", "Kasa", "Secilen LINE kasasina ait urun kodunu onaylayin."),
    ("5 / 7", "Temizlik", "Filtre temizlik sistemini secin."),
    ("6 / 7", "Pano", "Fan kumanda panosunu secin."),
    ("7 / 7", "Ozet", "Secilen LINE konfigurasyonunun ozetini inceleyin."),
]
...
_OPTION_ORDER = {
    ...
    "filter_variant": ["LINE.8", "LINE.12", "LINE.18", "LINE.24", "LINE.32", "LINE.36"],
...
_VARIANTS = [("LINE.8", 8), ("LINE.12", 12), ("LINE.18", 18), ("LINE.24", 24), ("LINE.32", 32), ("LINE.36", 36)]
...
def _recommended_fan_power(recommended_motor_kw):
    ...
    if recommended_motor_kw <= 11.0:
        return "11.0 kW"
    if recommended_motor_kw <= 15.0:
        return "15.0 kW"
    if recommended_motor_kw <= 18.5:
        return "18.5 kW"
    if recommended_motor_kw <= 22.0:
        return "22.0 kW"
    if recommended_motor_kw <= 30.0:
        return "30.0 kW"
    return None
...
def _filter_variant_options(state):
    ...
    result = []
    for variant, cartridge_count in _VARIANTS:
        section_area = _SECTION_AREAS_BY_VARIANT.get(variant)
        filter_area = 5.0 * cartridge_count
        ...
        result.append(
            {
                ...
                "product_code": _resolve_filter_product_code(state.get("filter_media"), cartridge_count),
            }
        )
    return result


def _resolve_filter_product_code(filter_media, cartridge_count):
...
def _resolve_fan_product_code(fan_type, fan_power):
    prefix = {"Plug Fan": "BRPF.DA.", "Salyangoz Fan": "BRF.DA."}.get(_normalize_text(fan_type))
    suffix = {
        "2.2 kW": "22.3000",
        "3.0 kW": "30.3000",
        "4.0 kW": "40.3000",
        ...
        "22.0 kW": "220.1500",
        "30.0 kW": "300.3000",
    }.get(_normalize_text(fan_power))
    if not prefix or not suffix:
        return None
    return prefix + suffix


def _cleaning_code_for_variant(filter_variant, cleaning):
    variant = _normalize_text(filter_variant)
    cleaning = _normalize_text(cleaning)
    if cleaning == "ECON":
        return {
            "LINE.8": "LINE.ECON.4",
            "LINE.12": "LINE.ECON.8",
            "LINE.18": "LINE.ECON.12",
            "LINE.24": "LINE.ECON.12",
            "LINE.32": "LINE.ECON.16",
            "LINE.36": "LINE.ECON.20",
        }.get(variant)
...
def _cleaning_description(filter_variant, cleaning):
    variant = _normalize_text(filter_variant)
    cleaning = _normalize_text(cleaning)
    if cleaning == "ECON":
        return {
            "LINE.8": "4 Cikisli Ekonomizer",
            "LINE.12": "8 Cikisli Ekonomizer",
            "LINE.18": "12 Cikisli Ekonomizer",
            "LINE.24": "12 Cikisli Ekonomizer",
            "LINE.32": "16 Cikisli Ekonomizer",
            "LINE.36": "20 Cikisli Ekonomizer",
        }.get(variant)
    if cleaning == "B-CONTROL":
        return {
            "LINE.8": "4-9 Cikisli LCD Panel",
            "LINE.12": "4-9 Cikisli LCD Panel",
            "LINE.18": "4-9 Cikisli LCD Panel",
            "LINE.24": "12-18 Cikisli LCD Panel",
            "LINE.32": "12-18 Cikisli LCD Panel",
            "LINE.36": "12-18 Cikisli LCD Panel",
        }.get(variant)
    return None
...
def _cleaning_options(filter_variant):
    return [
        {"title": "ECON", "product_code": _cleaning_code_for_variant(filter_variant, "ECON"), "description": _cleaning_description(filter_variant, "ECON")},
        {"title": "B-CONTROL", "product_code": _cleaning_code_for_variant(filter_variant, "B-CONTROL"), "description": _cleaning_description(filter_variant, "B-CONTROL")},
    ]
...
def _resolve_control_panel_product_code(option, fan_power):
    ...
    if option == "Motor Koruma Salteri":
        return {
            "2.2 kW": "LINE.MPS.380.50.22",
            "3.0 kW": "LINE.MPS.380.50.30",
            "4.0 kW": "LINE.MPS.380.50.40",
        }.get(fan_power)
    if option == "Yildiz Ucgen":
        return {
            "5.5 kW": "LINE.DS.380.50.55",
            "7.5 kW": "LINE.DS.380.50.75",
            "11.0 kW": "LINE.DS.380.50.110",
            "15.0 kW": "LINE.DS.380.50.150",
            "18.5 kW": "LINE.DS.380.50.185",
            "22.0 kW": "LINE.DS.380.50.220",
            "30.0 kW": "LINE.DS.380.50.300",
        }.get(fan_power)
...
def _resolve_runtime_metrics(state):
    ...
    filter_area = (5.0 * int(cartridge_count)) if cartridge_count else None
    rise_velocity = airflow_value / section_area / 3600.0 if airflow_value and section_area else None
    filtration_velocity = airflow_value / filter_area / 60.0 if airflow_value and filter_area else None
    return {
        "section_area": section_area,
...
def _build_summary(state):
    ...
    if not all([filter_variant, filter_media, cleaning, fan_type, fan_power, panel]):
        return None
    return {
        "combinationKey": "|".join([filter_variant, filter_media, cleaning, fan_type, fan_power, panel]),
        "kasa": filter_variant,
        ...
        "filtreSetKodu": _resolve_filter_product_code(filter_media, state.get("filter_cartridge_count")),
        "temizlikKodu": _cleaning_code_for_variant(filter_variant, cleaning),
        "fanKodu": _resolve_fan_product_code(fan_type, fan_power),
        "panoKodu": _resolve_control_panel_product_code(panel, fan_power),
        "selectionSummary": f"Kasa={filter_variant} | Filtre={filter_media} | Temizlik={cleaning} | Fan={fan_type} {fan_power} | Pano={panel}",
    }
...
def _compute_article_number(summary_row):
    if not summary_row:
        return None
    target = (
        _normalize_text(summary_row.get("kasa")),
        _normalize_text(summary_row.get("filtreMedyasi")),
        _normalize_text(summary_row.get("temizlik")),
        _normalize_text(summary_row.get("fanTipi")),
        _normalize_text(summary_row.get("fanGucu")),
        _normalize_text(summary_row.get("pano")),
    )
    article_index = 1
    for current_media in _FILTER_MEDIA:
        for current_variant, _count in _VARIANTS:
                ...
                for current_fan_power in _FAN_POWERS:
                    for current_panel in _control_panel_options(current_fan_power):
                        for current_cleaning in _ARTICLE_CLEANING_ORDER:
                            if target == (current_variant, current_media, current_cleaning, current_fan_type, current_fan_power, current_panel):
                                return f"D-LINE-{article_index:04d}"
                            article_index += 1
    return None
...
def _selection_rows(state):
    return [
        ...
        ("Filtre Boyu", _FIXED_FILTER_LENGTH),
        ("LINE Kasa", _normalize_text(state.get("filter_variant")) or "Secilmedi"),
    ...
    try:
        ...
        for regular_path, bold_path in [
            ...
            if os.path.exists(regular_path) and os.path.exists(bold_path):
                pdfmetrics.registerFont(TTFont("LinePdfFont", regular_path))
                pdfmetrics.registerFont(TTFont("LinePdfFontBold", bold_path))
                regular_font_name = "LinePdfFont"
                bold_font_name = "LinePdfFontBold"
                break
        ...
        pdf.drawString(40, y, _safe_pdf_text("LINE Secim Sihirbazi Ozeti"))
        y -= 28
        pdf.setFont(regular_font_name, 11)
...
def open_line_selection_wizard(parent=None, on_close=None):
    wizard = ctk.CTkToplevel(parent) if parent is not None else ctk.CTkToplevel()
    wizard.title("LINE Secim Sihirbazi")
    ...
    def render_summary_grid(parent_widget, items, columns=2, wraplength=240):
        ...
        for column_index in range(columns):
            grid_frame.grid_columnconfigure(column_index, weight=1, uniform="line_summary")
    ...
    def render_step():
        ...
        if current_step["value"] == 1:
            ...
            def add_result(row_index, label, key, emphasized=False):
                ...
                result_labels[key] = ctk.CTkLabel(row, text="-", font=ctk.CTkFont(size=13 if not emphasized else 14, weight="bold" if emphasized else "normal"), text_color="#d32f2f" if emphasized else "#4e342e", anchor="w", justify="left")
            ...
            def apply_criteria_and_continue(skip_validation=False):
                ...
                if airflow_value is None or airflow_value <= 0:
                    messagebox.showwarning("LINE Secim Sihirbazi", "Lutfen gecerli bir debi girin.")
                    return
                if pressure_value is None or pressure_value <= 0:
                    messagebox.showwarning("LINE Secim Sihirbazi", "Lutfen gecerli bir basinc girin.")
                    return
                if fan_efficiency_value is None or fan_efficiency_value <= 0 or fan_efficiency_value > 100:
                    messagebox.showwarning("LINE Secim Sihirbazi", "Lutfen gecerli bir fan verimi girin.")
                    return
                if temperature_value is None or temperature_value <= -273.15:
                    messagebox.showwarning("LINE Secim Sihirbazi", "Lutfen gecerli bir sicaklik girin.")
                    return
                if altitude_value is None or altitude_value >= 44330:
                    messagebox.showwarning("LINE Secim Sihirbazi", "Lutfen gecerli bir rakim girin.")
                    return
                if service_margin_value is None or service_margin_value < 0:
                    messagebox.showwarning("LINE Secim Sihirbazi", "Lutfen gecerli bir servis payi girin.")
                    return
                ...
                if recommended is None or shaft_power is None:
                    messagebox.showwarning("LINE Secim Sihirbazi", "Bu kriterler icin uygun bir LINE fan gucu bulunamadi.")
                    return
                ...
                if fan_efficiency_warning:
                    messagebox.showwarning("LINE Secim Sihirbazi", f"Uyari: {fan_efficiency_warning}")
                current_step["value"] = 2
                render_step()
            ...
            if state["filter_media"]:
                ...
                ctk.CTkLabel(variant_card, text="LINE Kasa", font=ctk.CTkFont(size=16, weight="bold"), text_color="#333333").pack(anchor="w", pady=(0, 10))
                option_group(
                    variant_card,
                    "filter_variant",
                    ...
                    option_description_builder=lambda option: (
                        f"Filtre Boyu: {_FIXED_FILTER_LENGTH}\n"
            ...
            ctk.CTkLabel(box, text="LINE tarafinda kasa urun kodu secilen varyant ile aynidir.", font=ctk.CTkFont(size=13), text_color="#666666").pack(anchor="w", pady=(8, 0))
            ...
            ctk.CTkLabel(top, text="LINE konfigurasyon ozeti", font=ctk.CTkFont(size=18, weight="bold"), text_color="#222222").pack(anchor="w", padx=16, pady=(14, 8))
            ...
            if summary:
                ...
                def open_product_document(document_kind, label):
                    try:
                        document = _find_series_document("LINE", document_kind)
                    except DocumentServiceError as exc:
                        messagebox.showerror("LINE Dokumanlari", f"Dokuman listesi alinamadi:\n{exc}")
                        return
                    if not document:
                        messagebox.showwarning("LINE Dokumanlari", f"{label} icin uygun bir dokuman bulunamadi.")
                        return
                    file_url = _normalize_text(document.get("file_url"))
                    if not file_url:
                        messagebox.showwarning("LINE Dokumanlari", f"{label} baglantisi bulunamadi.")
                        return
                    try:
                        webbrowser.open(file_url)
                    except Exception as exc:
                        messagebox.showerror("LINE Dokumanlari", f"Dokuman acilamadi:\n{exc}")
                ...
                def export_pdf():
                    default_name = f"LINE_{_normalize_text(state.get('filter_variant')).replace(' ', '_')}_{_normalize_text(state.get('fan_power')).replace(' ', '_')}_ozet.pdf"
                    cost_detail_rows = []
                    if cost_error:
                ...
                ctk.CTkLabel(content_card, text="Secilen adimlarla birebir eslesen bir LINE kombinasyonu bulunamadi.", font=ctk.CTkFont(size=14), text_color="#d32f2f").pack(anchor="w", padx=18, pady=(18, 14))
    ...
    def go_next():
        if current_step["value"] == 1 and (state.get("airflow_value") is None or state.get("pressure_value") is None):
            messagebox.showwarning("LINE Secim Sihirbazi", "Lutfen debi ve basinc bilgilerini girin.")
            return
        if current_step["value"] == 2 and (not state["fan_type"] or not state["fan_power"]):
            messagebox.showwarning("LINE Secim Sihirbazi", "Lutfen fan tipi ve fan gucunu secin.")
            return
        if current_step["value"] == 3 and (not state["filter_media"] or not state["filter_variant"]):
            messagebox.showwarning("LINE Secim Sihirbazi", "Lutfen filtre medyasi ve LINE kasa secimini tamamlayin.")
            return
        if current_step["value"] == 4 and not state["filter_variant"]:
            messagebox.showwarning("LINE Secim Sihirbazi", "Lutfen kasa urun kodunu onaylayin.")
            return
        if current_step["value"] == 5 and not state["cleaning"]:
            messagebox.showwarning("LINE Secim Sihirbazi", "Lutfen temizlik sistemi secin.")
            return
        if current_step["value"] == 6 and not state["panel"]:
            messagebox.showwarning("LINE Secim Sihirbazi", "Lutfen pano secin.")
            return
        ...
        if warnings:
            messagebox.showwarning("LINE Secim Sihirbazi", "\n\n".join(warnings))
