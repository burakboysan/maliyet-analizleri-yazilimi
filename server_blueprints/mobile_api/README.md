...
- `POST /desktop/ai-leads/apollo/segment-search`
...
Apollo Search ana strateji olarak kullanılacaksa `segment-search` endpoint'i önerilir. Kullanıcı manuel keyword veya unvan girmez; seçilen segment için `ai_search_recipes` tablosundaki reçete çalışır, sonuçlar enrich edilir, duplicate kayıtlar atlanır ve `ai_search_runs` tablosuna çalışma özeti yazılır.
