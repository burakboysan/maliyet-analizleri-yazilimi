...
def urun_ekle_ekrani(yenile_fonksiyonu=None, kullanici_rolu=None):
        ...
        def _on_success():
            ...
            if yenile_fonksiyonu:
                try:
                    ...
                    _debug(f"[ADD_PRODUCT] Yenile callback hatası: {cb_e}")
            pencere.destroy()

        def _run_insert():
            try:
                _debug("[ADD_PRODUCT] Kaydet kliklendi")
                _debug(f"[ADD_PRODUCT] Girdi özet: urun_kodu={urun_kodu}, urun_adi={urun_adi}, kategori={kategori}, tipi={tipi}, modeli={modeli}")
                _debug(f"[ADD_PRODUCT] Opsiyoneller: filtre_medyasi={filtre_medyasi}, filtre_kodu={filtre_medyasi_kodu}, patlac={patlac_kumanda_tipi}")
                _debug(f"[ADD_PRODUCT] Teknik: alan={toplam_filtre_alani}, debi={debi}, fan_basinc={fan_basinc} {fan_basinc_birimi}")
                _debug(f"[ADD_PRODUCT] Motor/kumanda: motor={motor}, kumanda={fan_kumanda_tipi}, patlama_kapagi={patlama_kapagi}, filtre_sayisi={filtre_elemani_sayisi}")
                _debug(f"[ADD_PRODUCT] Ayna: en={filtre_aynasi_eni}, boy={filtre_aynasi_boyu}, alan={filtre_aynasi_alani_deger}")

                _debug("[ADD_PRODUCT] DB bağlantısı alınıyor...")
                try:
                    is_conn = bool(baglanti and baglanti.is_connected())
                except Exception:
                    is_conn = False
                _debug(f"[ADD_PRODUCT] baglanti.is_connected={is_conn}")

                # Kilit beklemelerinde uzun süre donmayı engelle
                try:
                    cursor.execute("SET SESSION innodb_lock_wait_timeout = 5")
                except Exception as t_e:
                    _debug(f"[ADD_PRODUCT] lock_wait_timeout ayarlanamadı: {t_e}")
                ...
                try:
                    ...
                    if dupe_count > 0:
                        _debug(f"[ADD_PRODUCT] Duplicate urun_kodu: {urun_kodu}")
                        pencere.after(0, lambda: messagebox.showerror("Hata", f"Ürün kodu zaten mevcut: {urun_kodu}"))
                        return
                except Exception as dupe_e:
                    _debug(f"[ADD_PRODUCT] Duplicate kontrolü hatası: {dupe_e}")

                _debug("[ADD_PRODUCT] INSERT başlayacak")
                ...
                _debug(f"[ADD_PRODUCT] Dönüşümler -> alan={toplam_filtre_alani_val}, debi={debi_val}, fan={fan_basinc_val}, sayi={filtre_elemani_sayisi_val}")
                ...
                _debug("[ADD_PRODUCT] INSERT tamam, commit ediliyor...")
                baglanti.commit()
                _debug("[ADD_PRODUCT] Commit tamamlandı")
                pencere.after(0, _on_success)
            except Exception as e:
                tb = traceback.format_exc()
                _debug(f"[ADD_PRODUCT][HATA] {e}\n{tb}")
                try:
                    baglanti.rollback()
                    _debug("[ADD_PRODUCT] Rollback yapıldı")
                except Exception as rb_e:
                    _debug(f"[ADD_PRODUCT] Rollback başarısız: {rb_e}")
    ...
    _debug("[ADD_PRODUCT] Ekran açılıyor, DB bağlantısı oluşturulacak")
    baglanti = veritabani_baglanti()
    if not baglanti:
        _debug("[ADD_PRODUCT] Veritabanı bağlantısı kurulamadı (None döndü)")
        messagebox.showerror("Veritabanı", "Bağlantı kurulamadı.")
        return
    try:
        cursor = baglanti.cursor()
        _debug("[ADD_PRODUCT] Cursor oluşturuldu")
    except Exception as e:
        _debug(f"[ADD_PRODUCT] Cursor oluşturulamadı: {e}")
    ...
    combo_kategori = add_field(1, 0, "Kategori *", "combo", [
        "LEV", "KOMPAKT", "FİLTRE ÜNİTELERİ", "YAĞ BUHARI FİLTRELERİ",
        "RADYAL FANLAR", "ELEKTRİK PANOLARI", "BOA","FİLTRE SETLERİ",
        "DİĞER"
    ])
