...
def open_verty_wizard(parent=None, on_close=None):
    window = ctk.CTkToplevel(parent) if parent is not None else ctk.CTkToplevel()
    window.title("VERTY Secim Sihirbazi")
    ...
    def clear_after(key):
        ...
        if key in {"fan_type", "fan_power", "filter_media", "filter_length", "case", "cleaning", "fan_module", "sound", "panel", "dust"}:
            state["silencer"] = None

    def _fan_power_options():
        if not state["fan_type"]:
            return []
        options = list(legacy._VERTY_FAN_POWERS)
    ...
    def _filter_length_options():
        return legacy._verty_lengths_for_media(state["filter_media"])

    def _case_options():
        return legacy._verty_case_options(state["filter_length"])

    def _cleaning_options():
        return list(legacy._OPTION_ORDER["cleaning"])

    def _fan_module_options():
        return legacy._verty_fan_module_options(state["fan_type"], state["fan_power"], state["case"])

    def _sound_options():
        return legacy._sound_options_for_module(state["fan_module"])

    def _panel_options():
        return legacy._verty_panel_options(state["fan_power"])

    def _dust_options():
        return list(legacy._OPTION_ORDER["dust"])

    def _silencer_options():
    ...
    def normalize():
        ...
        if state["case"] not in _case_options():
            state["case"] = None
            clear_after("case")
        if state["cleaning"] not in _cleaning_options():
            state["cleaning"] = None
            clear_after("cleaning")
        if state["fan_module"] not in _fan_module_options():
            state["fan_module"] = None
            clear_after("fan_module")
        sounds = _sound_options()
        if len(sounds) == 1:
            state["sound"] = sounds[0]
        elif state["sound"] not in sounds:
            state["sound"] = None
            clear_after("sound")
        if state["panel"] not in _panel_options():
            state["panel"] = None
            clear_after("panel")
        if state["dust"] not in _dust_options():
            state["dust"] = None
        if state["silencer"] not in _silencer_options():
            state["silencer"] = None
    ...
    def set_value(key, value, refresh_left=True):
        ...
        if key == "fan_module" and len(_sound_options()) == 1:
            state["sound"] = _sound_options()[0]
        normalize()
        current["index"] = min(current["index"], len(steps()) - 1)
        if refresh_left or key in {"fan_type", "filter_media", "filter_length", "case", "fan_module"}:
    ...
    def apply_criteria(fan_excluded=False):
        ...
        if airflow is None or airflow <= 0:
            messagebox.showwarning("VERTY Sihirbazi", "Lutfen gecerli bir debi girin.")
            return
        if pressure is None or pressure <= 0:
            messagebox.showwarning("VERTY Sihirbazi", "Lutfen gecerli bir basinc girin.")
            return
        if fan_efficiency is None or fan_efficiency <= 0 or fan_efficiency > 100:
            messagebox.showwarning("VERTY Sihirbazi", "Lutfen gecerli bir fan verimi girin.")
            return
        if service_margin is None or service_margin < 0:
            messagebox.showwarning("VERTY Sihirbazi", "Lutfen gecerli bir servis payi girin.")
            return
        if temperature is None or temperature <= -273.15:
            messagebox.showwarning("VERTY Sihirbazi", "Lutfen gecerli bir sicaklik girin.")
            return
        if altitude is None or altitude >= 44330:
            messagebox.showwarning("VERTY Sihirbazi", "Lutfen gecerli bir rakim girin.")
            return
        ...
        recommended = legacy._recommended_fan_power(recommended_motor_kw)
        if recommended is None:
            messagebox.showwarning("VERTY Sihirbazi", "Bu kriterler icin uygun bir VERTY fan gucu bulunamadi.")
            return
        state.update(
            {
        ...
        normalize()
        if warning:
            messagebox.showwarning("VERTY Sihirbazi", f"Uyari: {warning}")
        current["index"] = 1
        render()
    ...
    def _render_left_content(parent, key, apply_criteria_fn, set_value_fn):
        ...
        if key == "criteria":
            ...
            _render_option_section(parent, "Kasa Secimi", "case", _case_options(), set_value_fn, state.get("case"), note=legacy._case_note(state["filter_length"]))
        elif key == "cleaning":
            _render_option_section(parent, "Temizlik Sistemi", "cleaning", _cleaning_options(), set_value_fn, state.get("cleaning"))
        elif key == "fan_module":
            _render_option_section(parent, "Fan Modulu", "fan_module", _fan_module_options(), set_value_fn, state.get("fan_module"))
        elif key == "sound":
            _render_option_section(parent, "Ses Izolasyonu", "sound", _sound_options(), set_value_fn, state.get("sound"))
        elif key == "panel":
            _render_option_section(parent, "Pano", "panel", _panel_options(), set_value_fn, state.get("panel"))
        elif key == "dust":
            _render_option_section(parent, "Toz Bosaltma", "dust", _dust_options(), set_value_fn, state.get("dust"))
        elif key == "silencer":
            _render_option_section(parent, "Susturucu", "silencer", _silencer_options(), set_value_fn, state.get("silencer"))
        else:
    ...
    def _render_summary(parent, key):
        ...
        summary = legacy._build_verty_summary(state)
        metrics = legacy._resolve_runtime_metrics([], state, summary)
    ...
    def _render_summary_actions(parent):
        summary = _build_summary_with_silencer(state)
        total, found, missing, zero, costs, error = _resolve_summary_cost_with_silencer(summary)
        article = legacy._resolve_verty_article_number(summary)
    ...
    def _export_pdf_from_state():
        summary = _build_summary_with_silencer(state)
        db_total_cost, found_cost_codes, missing_cost_codes, zero_cost_codes, costs_by_code, cost_error = _resolve_summary_cost_with_silencer(summary)
        article_number = legacy._resolve_verty_article_number(summary)

        code_rows = [
            ("Kasa Kodu", summary.get("kasa_kodu") if summary else None),
            ("Filtre Set Kodu", summary.get("filtre_set_kodu") if summary else None),
            ("Temizlik Kodu", summary.get("temizlik_kodu") if summary else None),
            ("Fan Kodu", summary.get("fan_kodu") if summary else None),
            ("Fan Modul Kodu", summary.get("fan_modul_kodu") if summary else None),
            ("Ses Izolasyon Kodu", summary.get("ses_izolasyon_kodu") if summary else None),
            ("Pano Kodu", summary.get("pano_kodu") if summary else None),
            ("Toz Bosaltma Kodu", summary.get("toz_bosaltma_kodu") if summary else None),
            ("Susturucu Kodu", summary.get("susturucu_kodu") if summary else None),
        ]
    ...
    def _validate_current_step(key):
        ...
        if any(not state.get(field) for field in checks.get(key, ())):
            messagebox.showwarning("VERTY Sihirbazi", "Lutfen bu adimdaki secimi tamamlayin.")
            return False
        return True
    ...
    def _render_criteria(parent, apply_criteria_fn):
        ...
        def refresh_preview(_event=None):
            ...
            try:
                ...
                recommended_text = legacy._recommended_fan_power(recommended_motor_kw) or "-"
    ...
    def _summary_cards_data():
        return [
            ...
            ("case", "Kasa", state["case"] or "Secilmedi", ""),
            ("cleaning", "Temizlik", state["cleaning"] or "Secilmedi", ""),
            ("panel", "Pano", state["panel"] or "Secilmedi", ""),
            ("dust", "Toz Bosaltma", state["dust"] or "Secilmedi", ""),
            ("silencer", "Susturucu", state["silencer"] or "Secilmedi", ""),
    ...
    def _codes_rows():
        ...
        if not summary:
            return []
        return [
            ("Filtre Kodu", summary.get("filtre_set_kodu")),
            ("Fan Kodu", summary.get("fan_kodu")),
            ("Kasa Kodu", summary.get("kasa_kodu")),
            ("Temizlik Kodu", summary.get("temizlik_kodu")),
            ("Pano Kodu", summary.get("pano_kodu")),
            ("Susturucu Kodu", summary.get("susturucu_kodu")),
        ]
...
def _build_summary_with_silencer(state):
    summary = legacy._build_verty_summary(state)
    if not summary or not state.get("silencer"):
        return summary
    enriched = dict(summary)
    enriched["susturucu"] = state.get("silencer")
    enriched["susturucu_kodu"] = _resolve_silencer_product_code(state.get("silencer"))
    return enriched


def _summary_product_codes_with_silencer(summary):
    if not summary:
        return []
    ordered_codes = [
        summary.get("kasa_kodu"),
        summary.get("filtre_set_kodu"),
        summary.get("temizlik_kodu"),
        summary.get("fan_kodu"),
        summary.get("fan_modul_kodu"),
        summary.get("ses_izolasyon_kodu"),
        summary.get("pano_kodu"),
        summary.get("toz_bosaltma_kodu"),
        summary.get("susturucu_kodu"),
    ]
    result = []
    seen = set()
    for code in ordered_codes:
        normalized = legacy._normalize_text(code).upper()
...
def _resolve_summary_cost_with_silencer(summary):
    product_codes = _summary_product_codes_with_silencer(summary)
    if not product_codes:
        return None, [], [], [], {}, None

    try:
        costs_by_code = legacy._fetch_product_costs_from_db(product_codes)
...
def _render_header(parent, title, index, total, back_command, next_command):
    ...
    ctk.CTkLabel(title_area, text="VERTY Secim Sihirbazi", font=ctk.CTkFont(size=24, weight="bold"), text_color="#111827").grid(row=0, column=0, sticky="w")
...
def _documents_card(parent, row):
    ...
    def open_product_document(document_kind, label):
        try:
            document = _find_verty_document(document_kind)
...
def _find_verty_document(document_kind):
    if document_kind in {"brosur", "teknik_foy", "tds"}:
        return legacy._find_series_document("VERTY", document_kind)
    documents = legacy._get_series_documents("VERTY")
    if not documents:
        return None
    candidates = [
...
def _render_header(parent, title, index, total, back_command, next_command):
    ...
    ctk.CTkLabel(title_area, text="VERTY Secim Sihirbazi", font=ctk.CTkFont(size=23, weight="bold"), text_color="#111827").grid(row=0, column=0, sticky="w")
