...
import hexafil_wizard as legacy
...
def open_hexafil_wizard(parent=None, on_close=None):
    window = ctk.CTkToplevel(parent) if parent is not None else ctk.CTkToplevel()
    window.title("HEXAFIL Secim Sihirbazi")
    ...
    def steps():
        ...
        if not state["is_fan_excluded"]:
            result.extend(
                [
                    ("fan_cabin", "Fan Kabini"),
                    ("sound", "Ses Izolasyonu"),
                    ("panel", "Pano"),
                    ("silencer", "Susturucu"),
                ]
            )
    ...
    def clear_after(key):
        for field_name in legacy._RESET_CHAIN.get(key, []):
            state[field_name] = None

    def _fan_power_options():
        if not state["fan_type"]:
            return []
        return legacy._available_fan_powers(state.get("type"), state["fan_type"], state["shaft_power"])
    ...
    def normalize():
        if state["is_fan_excluded"]:
            ...
            if state["fan_type"] not in legacy._available_fan_types(state["pressure_value"]):
                state["fan_type"] = None
                clear_after("fan_type")
            fan_options = _fan_power_options()
            if state["fan_power"] not in fan_options:
                state["fan_power"] = state["recommended_fan_power"] if state["recommended_fan_power"] in fan_options else None
                clear_after("fan_power")
            if state["fan_cabin"] not in legacy._available_fan_cabin_options(state["fan_type"]):
                state["fan_cabin"] = None
                state["sound"] = None
            sound_options = legacy._available_sound_options(state["fan_cabin"])
            if len(sound_options) == 1:
                state["sound"] = sound_options[0]
            elif state["sound"] not in sound_options:
                state["sound"] = None
            if state["panel"] not in legacy._available_control_panels(state["fan_power"]):
                state["panel"] = None
            if state["silencer"] not in legacy._SILENCER_OPTIONS:
                state["silencer"] = None

        if state["filter_length"] not in legacy._available_filter_lengths(state["filter_media"]):
            state["filter_length"] = None
            state["case"] = None
        if state["case"] not in legacy._available_cases(state["filter_length"]):
            state["case"] = None
        if state["type"] not in legacy._TYPE_OPTIONS:
            state["type"] = None
        if state["cleaning"] not in legacy._CLEANING_OPTIONS:
            state["cleaning"] = None
    ...
    def set_value(key, value, refresh_left=True):
        ...
        normalize()
        current["index"] = min(current["index"], len(steps()) - 1)
        if refresh_left or key in {"fan_type", "filter_media"}:
    ...
    def apply_criteria(fan_excluded=False):
        ...
        if airflow is None or airflow <= 0:
            messagebox.showwarning("HEXAFIL Sihirbazi", "Lutfen gecerli bir debi girin.")
            return
        if pressure is None or pressure <= 0:
            messagebox.showwarning("HEXAFIL Sihirbazi", "Lutfen gecerli bir basinc girin.")
            return
        if fan_efficiency is None or fan_efficiency <= 0 or fan_efficiency > 100:
            messagebox.showwarning("HEXAFIL Sihirbazi", "Lutfen gecerli bir fan verimi girin.")
            return
        if service_margin is None or service_margin < 0:
            messagebox.showwarning("HEXAFIL Sihirbazi", "Lutfen gecerli bir servis payi girin.")
            return
        if temperature is None or temperature <= -273.15:
            messagebox.showwarning("HEXAFIL Sihirbazi", "Lutfen gecerli bir sicaklik girin.")
            return
        if altitude is None or altitude >= 44330:
            messagebox.showwarning("HEXAFIL Sihirbazi", "Lutfen gecerli bir rakim girin.")
            return
        ...
        recommended = legacy._recommended_fan_power(state.get("type"), recommended_motor_kw)
        if not fan_excluded and recommended is None:
            messagebox.showwarning("HEXAFIL Sihirbazi", "Bu kriterler icin uygun bir HEXAFIL fan gucu bulunamadi.")
            return
        state.update(
            {
    ...
    def _render_left_content(parent, key, apply_criteria_fn, set_value_fn):
        ...
        if key == "criteria":
            ...
            _render_option_section(parent, "Fan Tipi", "fan_type", legacy._available_fan_types(state["pressure_value"]), set_value_fn, state.get("fan_type"))
            ...
            _render_option_section(parent, "Filtre Medyasi", "filter_media", list(legacy._FILTER_MEDIA_OPTIONS), set_value_fn, state.get("filter_media"))
            _render_option_section(parent, "Filtre Boyu", "filter_length", legacy._available_filter_lengths(state["filter_media"]), set_value_fn, state.get("filter_length"))
        elif key == "case":
            _render_option_section(parent, "Kasa Secimi", "case", legacy._available_cases(state["filter_length"]), set_value_fn, state.get("case"), note=legacy._case_note(state["filter_length"]))
        elif key == "type":
            _render_option_section(parent, "Tip Secimi", "type", list(legacy._TYPE_OPTIONS), set_value_fn, state.get("type"))
        elif key == "cleaning":
            _render_option_section(parent, "Temizlik Sistemi", "cleaning", list(legacy._CLEANING_OPTIONS), set_value_fn, state.get("cleaning"))
        elif key == "fan_cabin":
            _render_option_section(parent, "Fan Kabini", "fan_cabin", legacy._available_fan_cabin_options(state["fan_type"]), set_value_fn, state.get("fan_cabin"))
        elif key == "sound":
            _render_option_section(parent, "Ses Izolasyonu", "sound", legacy._available_sound_options(state["fan_cabin"]), set_value_fn, state.get("sound"))
        elif key == "panel":
            _render_option_section(parent, "Pano", "panel", legacy._available_control_panels(state["fan_power"]), set_value_fn, state.get("panel"))
        elif key == "silencer":
            _render_option_section(parent, "Susturucu", "silencer", list(legacy._SILENCER_OPTIONS), set_value_fn, state.get("silencer"))
        else:
    ...
    def _render_summary_actions(parent):
        rows = legacy._build_summary_code_rows(state)
        total, found, missing, zero, costs, error = legacy._resolve_summary_cost(rows)
        article = legacy._resolve_hexafil_article_number(state)
        ...
        ).pack(anchor="w", padx=18, pady=(14, 16))
    ...
    def _selection_rows():
        return [
            ...
            ("Fan Tipi", "HARIC" if state["is_fan_excluded"] else state["fan_type"] or "-"),
            ("Fan Gucu", "HARIC" if state["is_fan_excluded"] else state["fan_power"] or "-"),
            ("Filtre Medyasi", state["filter_media"] or "-"),
            ("Filtre Boyu", state["filter_length"] or "-"),
            ("Kasa", state["case"] or "-"),
            ("Tip", state["type"] or "-"),
            ("Temizlik", state["cleaning"] or "-"),
            ("Fan Kabini", "HARIC" if state["is_fan_excluded"] else state["fan_cabin"] or "-"),
            ("Ses Izolasyonu", "HARIC" if state["is_fan_excluded"] else state["sound"] or "-"),
            ("Pano", "HARIC" if state["is_fan_excluded"] else state["panel"] or "-"),
            ("Susturucu", "HARIC" if state["is_fan_excluded"] else state["silencer"] or "-"),
        ]
    ...
    def _export_pdf_from_state():
        code_rows = legacy._build_summary_code_rows(state)
        db_total_cost, found_cost_codes, missing_cost_codes, zero_cost_codes, costs_by_code, cost_error = legacy._resolve_summary_cost(code_rows)
        article_number = legacy._resolve_hexafil_article_number(state)
    ...
    def _validate_current_step(key):
        ...
        if any(not state.get(field) for field in checks.get(key, ())):
            messagebox.showwarning("HEXAFIL Sihirbazi", "Lutfen bu adimdaki secimi tamamlayin.")
            return False
        return True
    ...
    def _render_criteria(parent, apply_criteria_fn):
        ...
        def refresh_preview(_event=None):
            ...
            try:
                ...
                preliminary_motor_kw = legacy._recommended_motor_power_from_criteria(
                    airflow_value,
                    pressure_value,
                ...
                recommended_text = legacy._recommended_fan_power(state.get("type"), recommended_motor_kw) or "-"
        ...
        ctk.CTkButton(actions, text="Fansiz Devam", command=lambda: apply_criteria_fn(True), fg_color="#ffffff", hover_color="#f3f4f6", text_color=TEXT, border_width=1, border_color=BORDER, height=38, corner_radius=6).pack(side="left")
    ...
    def _summary_cards_data():
        return [
            ...
            ("fan", "Fan Gucu", ("HARIC" if state["is_fan_excluded"] else state["fan_power"]) or "Secilmedi", "Nominal Motor Gucu"),
            ("case", "Kasa", state["case"] or "Secilmedi", state["type"] or ""),
            ("cleaning", "Temizlik", state["cleaning"] or "Secilmedi", ""),
            ("panel", "Pano", ("HARIC" if state["is_fan_excluded"] else state["panel"]) or "Secilmedi", ""),
...
def _render_header(parent, title, index, total, back_command, next_command):
    ...
    ctk.CTkLabel(title_area, text="HEXAFIL Secim Sihirbazi", font=ctk.CTkFont(size=24, weight="bold"), text_color="#111827").grid(row=0, column=0, sticky="w")
...
def _documents_card(parent, row):
    ...
    def open_product_document(document_kind, label):
        try:
            document = _find_hexafil_document(document_kind)
...
def _find_hexafil_document(document_kind):
    if document_kind in {"brosur", "teknik_foy", "tds"}:
        return legacy._find_series_document("HEXAFIL", document_kind)
    documents = legacy._get_series_documents("HEXAFIL")
    if not documents:
        return None
    candidates = [
...
def _render_header(parent, title, index, total, back_command, next_command):
    ...
    ctk.CTkLabel(title_area, text="HEXAFIL Secim Sihirbazi", font=ctk.CTkFont(size=23, weight="bold"), text_color="#111827").grid(row=0, column=0, sticky="w")
...
def _render_option_section(parent, title, field, options, set_value, selected_value=None, note=None):
    ...
    for option in options:
        ...
        ctk.CTkLabel(text_frame, text=option, font=ctk.CTkFont(size=18), text_color="#111827").pack(anchor="w")
        if desc:
        ...
        card_handles.append((option, card, circle, check))

        for widget in (card, circle, text_frame):
            widget.bind("<Button-1>", lambda _event, value=option: select_option(value))
        for child in text_frame.winfo_children():
            child.bind("<Button-1>", lambda _event, value=option: select_option(value))
