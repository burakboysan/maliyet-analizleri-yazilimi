...
def open_ecog_wizard(parent=None, on_close=None):
    ...
    window.title("ECOG Secim Sihirbazi")
    ...
    def _filter_length_options():
        return list(legacy._OPTION_ORDER["filter_length"])

    def _case_options():
        if not state["filter_media"] or not state["filter_length"]:
            return []
        return list(legacy._OPTION_ORDER["filter_variant"])

    def _cleaning_options():
        return list(legacy._OPTION_ORDER["cleaning"])

    def _panel_options():
        return legacy._available_control_panels(state["fan_power"])
    ...
    def normalize():
        ...
        elif state["filter_variant"]:
            state["case_code"] = legacy._resolve_case_product_code(state["filter_variant"], state["filter_length"])
    ...
    def apply_criteria(fan_excluded=False):
        ...
        if airflow is None or airflow <= 0:
            messagebox.showwarning("ECOG Sihirbazi", "Lutfen gecerli bir debi girin.")
            return
        if pressure is None or pressure <= 0:
            messagebox.showwarning("ECOG Sihirbazi", "Lutfen gecerli bir basinc girin.")
            return
        if fan_efficiency is None or fan_efficiency <= 0 or fan_efficiency > 100:
            messagebox.showwarning("ECOG Sihirbazi", "Lutfen gecerli bir fan verimi girin.")
            return
        if service_margin is None or service_margin < 0:
            messagebox.showwarning("ECOG Sihirbazi", "Lutfen gecerli bir servis payi girin.")
            return
        if temperature is None or temperature <= -273.15:
            messagebox.showwarning("ECOG Sihirbazi", "Lutfen gecerli bir sicaklik girin.")
            return
        if altitude is None or altitude >= 44330:
            messagebox.showwarning("ECOG Sihirbazi", "Lutfen gecerli bir rakim girin.")
            return
        ...
        if recommended is None:
            messagebox.showwarning("ECOG Sihirbazi", "Bu kriterler icin uygun bir ECOG fan gucu bulunamadi.")
            return
        state.update(
            {
        ...
        normalize()
        if warning:
            messagebox.showwarning("ECOG Sihirbazi", f"Uyari: {warning}")
        current["index"] = 1
        render()
    ...
    def _render_left_content(parent, key, apply_criteria_fn, set_value_fn):
        ...
        if key == "criteria":
            ...
            _render_option_section(parent, "Kasa Secimi", "filter_variant", _case_options(), set_value_fn, state.get("filter_variant"), note=_case_note())
        elif key == "cleaning":
    ...
    def _render_summary(parent, key):
        ...
        summary = legacy._build_ecog_summary(state)
    ...
    def _render_summary_actions(parent):
        summary = legacy._build_ecog_summary(state)
        total, found, missing, zero, costs, error = legacy._resolve_summary_cost(summary, state)
        article = legacy._resolve_ecog_article_number(summary)
    ...
    def _export_pdf_from_state():
        summary = legacy._build_ecog_summary(state)
        db_total_cost, found_cost_codes, missing_cost_codes, zero_cost_codes, costs_by_code, cost_error = legacy._resolve_summary_cost(summary, state)
        article_number = legacy._resolve_ecog_article_number(summary)
    ...
    def _validate_current_step(key):
        ...
        if any(not state.get(field) for field in checks.get(key, ())):
            messagebox.showwarning("ECOG Sihirbazi", "Lutfen bu adimdaki secimi tamamlayin.")
            return False
        return True
    ...
    def _summary_cards_data():
        return [
            ...
            ("case", "Kasa", state["filter_variant"] or "Secilmedi", state["case_code"] or ""),
...
def _case_note():
    return "Secilen filtre boyu ve medya icin ECOG kasa alternatiflerini degerlendirin."
...
def _render_header(parent, title, index, total, back_command, next_command):
    ...
    ctk.CTkLabel(title_area, text="ECOG Secim Sihirbazi", font=ctk.CTkFont(size=24, weight="bold"), text_color="#111827").grid(row=0, column=0, sticky="w")
...
def _documents_card(parent, row):
    ...
    def open_product_document(document_kind, label):
        try:
            document = _find_ecog_document(document_kind)
...
def _find_ecog_document(document_kind):
    if document_kind in {"brosur", "teknik_foy", "tds"}:
        return legacy._find_series_document("ECOG", document_kind)
    documents = legacy._get_series_documents("ECOG")
    if not documents:
        return None
    candidates = [
...
def _render_header(parent, title, index, total, back_command, next_command):
    ...
    ctk.CTkLabel(title_area, text="ECOG Secim Sihirbazi", font=ctk.CTkFont(size=23, weight="bold"), text_color="#111827").grid(row=0, column=0, sticky="w")
