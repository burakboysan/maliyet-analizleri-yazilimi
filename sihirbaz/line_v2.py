...
_PDF_IMAGE_CACHE = {}
_RESET_CHAIN = {
    "fan_type": ["fan_power", "filter_media", "filter_variant", "filter_cartridge_count", "cleaning", "panel"],
    "fan_power": ["filter_media", "filter_variant", "filter_cartridge_count", "cleaning", "panel"],
    "filter_media": ["filter_variant", "filter_cartridge_count", "cleaning", "panel"],
...
def open_line_wizard(parent=None, on_close=None):
    ...
    window.title("LINE Secim Sihirbazi")
    ...
    def set_value(key, value, refresh_left=True):
        ...
        normalize()
        current["index"] = min(current["index"], len(steps()) - 1)
        if refresh_left or key in {"fan_type", "filter_media", "filter_variant"}:
    ...
    def apply_criteria(fan_excluded=False):
        ...
        if airflow is None or airflow <= 0:
            messagebox.showwarning("LINE Sihirbazi", "Lutfen gecerli bir debi girin.")
            return
        if pressure is None or pressure <= 0:
            messagebox.showwarning("LINE Sihirbazi", "Lutfen gecerli bir basinc girin.")
            return
        if fan_efficiency is None or fan_efficiency <= 0 or fan_efficiency > 100:
            messagebox.showwarning("LINE Sihirbazi", "Lutfen gecerli bir fan verimi girin.")
            return
        if service_margin is None or service_margin < 0:
            messagebox.showwarning("LINE Sihirbazi", "Lutfen gecerli bir servis payi girin.")
            return
        if temperature is None or temperature <= -273.15:
            messagebox.showwarning("LINE Sihirbazi", "Lutfen gecerli bir sicaklik girin.")
            return
        if altitude is None or altitude >= 44330:
            messagebox.showwarning("LINE Sihirbazi", "Lutfen gecerli bir rakim girin.")
            return
        ...
        if recommended is None:
            messagebox.showwarning("LINE Sihirbazi", "Bu kriterler icin uygun bir LINE fan gucu bulunamadi.")
            return
        state.update(
            {
        ...
        normalize()
        if warning:
            messagebox.showwarning("LINE Sihirbazi", f"Uyari: {warning}")
        current["index"] = 1
        render()
    ...
    def _validate_current_step(key):
        ...
        if any(not state.get(field) for field in checks.get(key, ())):
            messagebox.showwarning("LINE Sihirbazi", "Lutfen bu adimdaki secimi tamamlayin.")
            return False
        return True
...
def _case_note():
    return "Secilen filtre medyasi ve debi bilgisine gore LINE kasa alternatiflerini degerlendirin."
...
def _render_header(parent, title, index, total, back_command, next_command):
    ...
    ctk.CTkLabel(title_area, text="LINE Secim Sihirbazi", font=ctk.CTkFont(size=24, weight="bold"), text_color="#111827").grid(row=0, column=0, sticky="w")
...
def _metric_card(parent, row, column, title, value, status, helper):
    ...
    status_icon = "OK" if status_key == "green" else "!" if status_key == "yellow" else "X" if status_key == "red" else ""
...
def _documents_card(parent, row):
    ...
    def open_product_document(document_kind, label):
        try:
            document = _find_line_document(document_kind)
...
def _find_line_document(document_kind):
    if document_kind in {"brosur", "teknik_foy", "tds"}:
        return legacy._find_series_document("LINE", document_kind)
    documents = legacy._get_series_documents("LINE")
    if not documents:
        return None
    candidates = [
...
def _render_header(parent, title, index, total, back_command, next_command):
    ...
    ctk.CTkLabel(title_area, text="LINE Secim Sihirbazi", font=ctk.CTkFont(size=23, weight="bold"), text_color="#111827").grid(row=0, column=0, sticky="w")
...
def _render_option_section(parent, title, field, options, set_value, selected_value=None, note=None):
    ...
    for option in normalized_options:
        ...
        check = ctk.CTkLabel(card, text="OK" if is_selected else "", font=ctk.CTkFont(size=16, weight="bold"), text_color=GREEN)
